...
*Cycle Inspection: Re-inspection* data from `r rest_inspec %>% pull(camis) %>% n_distinct()` unique restaurants in NYC throughout 2016.
...
rest_inspec %>%
  ...
  summarise(cuisine_type = mean(score),
            cuisine_avg_score = str_c("Average Score: ", cuisine_type)) %>%
  mutate(cuisine_description = fct_reorder(cuisine_description, cuisine_avg_score)) %>%
  plot_ly(
    x = ~cuisine_description, y = ~cuisine_type,
    type = "bar", mode = "markers", colors = "viridis",
    alpha = 0.5, text = ~cuisine_avg_score)  %>%
