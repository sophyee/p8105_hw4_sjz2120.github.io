...
In this flexdashboard, we'll be looking at restaurant inspection scores and dates in the year 2016 for restaurants in Manhattan.
...
rest_inspec =
  rest_inspec %>%
  select(
    camis, boro, critical_flag, cuisine_description, inspection_date, inspection_type, score, violation_code, grade) %>%
  filter(
    score >= 0,
...
There are `r nrow(rest_inspec)` restaurant inspection observations in the subsetted dataset we'll be using for the Flexdashboard, with 
*Cycle Inspection: Re-inspection* data from `r rest_inspec %>% pull(camis) %>% n_distinct()` unique restaurants in Manhattan throughout 2016.
